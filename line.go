...
func (self *Line) Rasterize(callback RasterizeCallback) {
	...
	if dir.X == 0 {
		...
		for y:=sy;y!=ey;y+=step {if !callback(x,y) {return}}
		return
	}
	...
	if dir.Y == 0 {
		...
		for x:=sx;x!=ex;x+=step {if !callback(x,y) {return}}
		return
	}
	...
	if dir.X >=0 {xDir = RIGHT} else {xDir = LEFT}
	if dir.Y >=0 {yDir = UP} else {yDir = DOWN}
